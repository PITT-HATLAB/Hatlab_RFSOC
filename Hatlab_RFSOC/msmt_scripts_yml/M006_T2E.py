...
if __name__ == "__main__":
    ...
    # plot IQ result
    plotData.plotAvgIQresults(sweepTime, avgi, avgq, title="T2E",xlabel="time (us)", ylabel="Qubit IQ", ro_chs=[ADC_idx])

    # fit result
    t2eDecay = qfr.T1Decay(sweepTime, avgi[ADC_idx][0] + 1j * avgq[ADC_idx][0])
    t2eresult = t2eDecay.run(info["rotResult"])
    t2eresult.plot()


    # save data to ddh5
    quick_save(info["dataPath"], f"{info['sampleName']}_T2E", avgi, avgq, config=config, sweepTime=sweepTime)
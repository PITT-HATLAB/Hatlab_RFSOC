...
class CavityResponseProgram_tanh(QubitMsmtMixin, NDAveragerProgram):
    def initialize(self):
        cfg = self.cfg
        self.res_ch = self.cfg["gen_chs"]["res_drive"]["ch"]

        # set readout pulse registers
        self.add_waveform_from_cfg("res_drive", "res_tanh")
        self.set_pulse_params_auto_gen_type("res_drive", **cfg["res_pulse_config"])
...
class PulseSpecProgram(QubitMsmtMixin, NDAveragerProgram):
    def initialize(self):
        ...
        self.q_r_freq = self.get_gen_reg("q_drive", "freq")
        self.q_r_freq_update = self.new_gen_reg("q_drive", reg_type="freq")
    ...
    def body(self):
        ...
        self.set_pulse_params("q_drive", style="const", length=cfg["prob_length"],
                                 phase=0, freq=cfg["f_start"], gain=cfg["prob_gain"])
        self.q_r_freq.set_to(self.q_r_freq_update)  # set the updated freq value
        self.pulse(ch=self.qubit_ch)  # play prob pulse
        self.sync_all(self.us2cycles(0.05))  # ali
        # gn channels and wait 50ns
...
class AmplitudeRabiProgram(QubitMsmtMixin, NDAveragerProgram):
    def initialize(self):
        ...
        self.q_r_gain = self.get_gen_reg("q_drive", "gain")
        self.q_r_gain_update = self.new_gen_reg("q_drive", init_val=cfg["g_start"])
    ...
    def body(self):
        ...
        # drive and measure
        self.q_r_gain.set_to(self.q_r_gain_update)  # set the updated gain value
        self.pulse(ch=self.qubit_ch)  # play gaussian pulse
...
class T1Program(QubitMsmtMixin, NDAveragerProgram):
    def initialize(self):
        ...
        self.t_r_wait = self.new_gen_reg("q_drive", init_val=cfg["t_start"], reg_type="time", tproc_reg=True)
...
class EfRabiProgram(QubitMsmtMixin, NDAveragerProgram):
    def initialize(self):
        ...
        self.q_r_gain = self.get_gen_reg("q_drive", "gain")
        self.q_r_gain_update = self.new_gen_reg("q_drive")
        self.add_sweep(QickSweep(self, self.q_r_gain_update, cfg["g_start"], cfg["g_stop"], cfg["g_expts"]))

...
class ChirpModulation:
    def __init__(self, chirp_func, maxf, maxv):
        self.maxf = maxf
        self.maxv = maxv
...
class WaveformCorrection:
    def __init__(self, filepath, freq):
        self.freq = freq
        self.calibFilePath = filepath
        self.calibration_func = self.get_calibration_func(self._get_calib_data(self.calibFilePath))
    ...
    @staticmethod
    def apply_modulation(signal, calibration_func, sampling_rate):
        """
        Modify a waveform in the frequency domain and return the modified time-domain waveform.
...
class Gaussian(Waveform):
    def __init__(self, prog, gen_ch, length, sigma, phase=0, maxv=30000,
...
class TanhBox(Waveform):
    def __init__(self, prog, gen_ch, length, ramp_width, cut_offset=0.01, phase=0, maxv=30000,
                 padding: Union[float, List[float], None] = None):
        super().__init__(prog, gen_ch, phase=phase, maxv=maxv)
...
class GaussianModulated(Waveform):
    def __init__(self, prog, gen_ch, length, sigma, phase=0, maxv=30000, modulations: list = (),
...
class TanhBoxModulated(Waveform):
    def __init__(self, prog, gen_ch, length, ramp_width, cut_offset=0.01, phase=0, maxv=30000, modulations: list = (),
                 padding: Union[float, List[float], None] = None, shape=None):
        super().__init__(prog, gen_ch, phase=phase, maxv=maxv)
        self.ramp_samps = self.us_to_samps(ramp_width)
        self.length_samps = self.us_to_samps(length)
...
class ConcatenateWaveform(Waveform):
    def __init__(self, prog, gen_ch, waveforms: List[Waveform], phase=0, maxv=30000, shape=None):
...
def add_waveform(prog: QickProgram, gen_ch, name, shape, **kwargs):
    ...
    if shape.lower() in (wave.lower() for wave in WaveformRegistry.available_waveforms()):
        ...
        raise NameError(f"unsupported pulse shape {shape}."
                        f" The available shapes are {WaveformRegistry.available_waveforms()}")
...
if __name__ == "__main__":
    ...
    wf = TanhBox(prog, 0, length=0.05, ramp_width=0.01, phase=0, padding=[0.015, 0.015])
    wf.plot_waveform()
    ...
    wf2 = GaussianModulated(prog=prog, gen_ch=0, length=0.05, sigma=0.01, phase=0, padding=[0.015, 0.015],
                            modulations=[dm, cm], name="GaussianChirpDrag")
    wf2 = TanhBoxModulated(prog=prog, gen_ch=0, length=0.05, ramp_width=0.01, phase=0, padding=[0.015, 0.015],
                           modulations=[dm, cm], name="TanhboxChirpDrag")
    wf2.plot_waveform()

    wfc = ConcatenateWaveform(prog=prog, gen_ch=0, waveforms=[wf, wf2], phase=0, name="concatenated_pulse_1")
    wfc.plot_waveform()
